		...
		protected virtual void DrawFiltersList()
		{
			...
			foreach (var filter in filters)
			{
				using (new GUILayout.HorizontalScope(UIHelpers.panelWithBackground))
				{
					...
					{
						filter.enabled = EditorGUILayout.Toggle(GUIContent.none, filter.enabled, GUILayout.Width(12));
						if (scope.changed)
						{
							SaveChanges();
						}
						...
						if (!filter.enabled)
						{
							GUI.contentColor = EditorStyles.centeredGreyMiniLabel.active.textColor;
						}
					}
