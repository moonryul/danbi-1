...
class SimpleHolographicCamera : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (kinectManager && kinectManager.IsInitialized()) 
		{
			...
			{
                jointHeadPos = GetCameraPosition(userId); // kinectManager.GetJointPosition (userId, (int)KinectInterop.JointType.Head);
...
//				initialRelPos = Vector3.zero;
    ...
    private Vector3 GetCameraPosition(long userId)
    {
        Vector3 headPos = kinectManager.GetJointPosition(userId, (int)KinectInterop.JointType.Head);
        Quaternion headRot = initialHeadRot * kinectManager.GetJointOrientation(userId, (int)KinectInterop.JointType.Head, false); // mirrored rotation
