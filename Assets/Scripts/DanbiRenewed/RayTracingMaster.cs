...
public class RayTracingMaster : MonoBehaviour
{
    ...
    public RenderTexture _mainScreenRT; // added by Moon Jung
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            ScreenCapture.CaptureScreenshot(Time.time + "-" + _currentSample + ".png", superSize);
    ...
    void InitRenderTexture()
    {
        ...
        // if (_target == null || _target.width != ScreenWidth || _target.height != ScreenHeight)
        {
            ...
            _converged = new RenderTexture(ScreenWidth, ScreenHeight, 0,
            RenderTextureFormat.ARGBFloat, RenderTextureReadWrite.Linear);
    ...
    void Render(RenderTexture destination)
    {
        ...
        Graphics.Blit(_converged, _mainScreenRT);
        Graphics.Blit(_mainScreenRT, destination);
        //Graphics.Blit(_converged, destination);
        //Blit sets destination  as the render target ( SV_Target), sets source (
        //_converged)  as _MainTex property  on the material and draws a full-screen quad to which
        // the default uv map is defined between the quad and _MainTex.

        // if the camera's targetTexture is not null, 
        // the the source texture will be written on  it, even if the destination is set to null.
        // So Set Camera.main.targetTexture to null before calling to Blit to  the framebuffer
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        // source refers to what the camera has rendered so far.
    ...
    void DebugLogOfRWBuffers()
    {
        ...
        for (int y = 0; y < Screen.height; y += 10)
        {
            for (int x = 0; x < Screen.width; x += 10)
            {
                ...
                Debug.Log("(" + x + "," + y + "):" + "current attenudated ray energy=" + accumRayEnergy.ToString("F6"));
