...
Properties {
	...
	_CylinderHeight ("Cylinder Height", float ) = 0
}
...
SubShader {
    ...
    Pass {
...
#ifndef _MAPPING_6_FRAMES_LAYOUT
        inline float2 ToRadialCoords(float3 coords) // coords -> 3d coords vertex 
        {
			/////////////////////////////////////////////////////////////////////
			...
			// acos(0) = PI / 2
			// acos(-1) = PI
			   float latitude = acos(normalizedCoords.y); // phi = arcsin (y / r), r = sqrt(x^2 + y^2 + x^2), coords =(x,y,z)
			// atan2 range : -PI ~ PI        평면상의 사분면을 판단
			   float longitude = atan2(normalizedCoords.z, normalizedCoords.x); // l = arctan( z/x)
        ...
        {
            v2f o;
			// 인스턴스 ID가 셰이더 함수에 액세스할 수 있게 만들 수 있게함.
			// 버텍스 셰이더의 맨 처음에 사용해야함
			UNITY_SETUP_INSTANCE_ID(v);
			
			// unity_StereoEyeIndex 값을 기반으로 텍스처 배열에서 어느 쪽 눈에 렌더링되어야 하는지 GPU에 알림.
			// vert 의 unity_StereoEyeIndex 값을 전송하여  frag 에서 UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX가 호출된 경우에만 
			// 프래그먼트 셰이더에서 액세스할 수 있도록 만듬
            UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o);


            float3 rotated = RotateAroundYInDegrees(v.vertex, _Rotation);
            o.vertex = UnityObjectToClipPos(rotated);
			// 표준과 다르게 텍스쳐의 uv를 주지않음
			// this mean (v.vertex.xyz) will be use (o.texcoord) field 
            ...
            if (aspectTest[0]) // horizontal
            {
                if (aspectTest[2])
                { // horizontal strip
                    o.faceXCoordLayouts = float4(0.5,0.5,1.5,0.5); // x축면의 좌표
                    o.faceYCoordLayouts = float4(2.5,0.5,3.5,0.5);
                    o.faceZCoordLayouts = float4(4.5,0.5,5.5,0.5);
                    o.layout = float3(-1,1.0/6.0,1.0/1.0); // x : bool -> 세로 교차 레이아웃 표시, yz : 정규화 좌표를 텍스쳐 좌표로 조정하기 위한 값
                }
                else
                { // horizontal cross
...
#else
            float2 tc = ToRadialCoords(i.texcoord); // i.texcoord : 카메라 좌표상의 삼각형 내부의 임의의 점의 좌표
